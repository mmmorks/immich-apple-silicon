...
| **ML: Face detection** | ONNX Runtime (SCRFD) | Apple Vision framework (Neural Engine) | Different model entirely. Detection accuracy is comparable but the detected *set* of faces differs (Vision recall ≈85% of SCRFD on frontal faces, lower on profiles/occlusion); bounding boxes and confidence calibration differ, so `ML_FACE_MIN_SCORE` is not the same operating point as Docker's threshold. |
| **ML: Face recognition** | ONNX Runtime | ONNX Runtime with CoreML | Same ArcFace model (`buffalo_l` / `w600k_r50`), bit-identical 5-point `norm_crop` alignment (our `face_embed._norm_crop` reproduces insightface's similarity warp on skimage's current `SimilarityTransform.from_estimate`, verified equal to 0 ULP), CoreML acceleration. Two divergences: (1) the 5-point landmarks fed into alignment (Vision contours vs SCRFD keypoints) drift the embedding for the same face by a measured median cosine ≈0.98 — small enough that **the existing face index/clusters are preserved (no re-scan needed)**; (2) on degenerate landmarks too collinear to align, the native path skips the face rather than warping a NaN matrix into a garbage crop as insightface silently does. See `ml/scripts/face_embedding_parity.py`. |
...
- **Face grouping**: Faces are detected and grouped correctly, and embeddings stay compatible with a Docker-built face index — a face re-detected natively lands at cosine ≈0.98 of its stored vector with no measured identity-retrieval loss, so **migrating to the native worker does not require re-running face recognition**. Grouping boundaries may differ slightly (a borderline face might be grouped differently), and Apple Vision detects a somewhat different set of faces than Docker's SCRFD detector, so coverage — which faces get detected — can differ going forward.
