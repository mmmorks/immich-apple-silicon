...
| **ML: Face recognition** | ONNX Runtime | ONNX Runtime with CoreML | Same ArcFace model (`buffalo_l` / `w600k_r50`), same `norm_crop` alignment, CoreML acceleration. The only divergence is the 5-point landmarks fed into alignment (Vision contours vs SCRFD keypoints), which drifts the embedding for the same face by a measured median cosine ≈0.98 — small enough that **the existing face index/clusters are preserved (no re-scan needed)**. See `ml/scripts/face_embedding_parity.py`. |
